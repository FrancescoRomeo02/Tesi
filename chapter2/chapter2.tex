...
% -------------------------------------------------------------
Intré S.r.l.\footnote{\url{https://www.intre.it}} \todo{perché footnote se hai una sitografia?} è una software factory
...
rapidi di apprendimento di gruppo, della durata di circa quattro mesi, costruiti a
...
puramente parametrica~\cite{lewis2020rag}, e deve rendere più rapido il recupero
...
\begin{table}[ht]
  ...
  \begin{tabular}{L{2.7cm}L{4.1cm}L{3cm}L{3.6cm}}
    ...
    Embedding denso & Document/text embedder denso &
      \makecell[l]{\texttt{sentence-}\\\texttt{transformers/}\\\texttt{paraphrase-}\\\texttt{multilingual-}\\\texttt{mpnet-base-v2}} &
...
mentre FastEmbed produce gli embedding in un job separato. La componente sparsa
rende inoltre il retrieval meno dipendente dalla sola similarità densa, aspetto
rilevante in un corpus dove nomi propri, tecnologie e termini ricorrenti hanno
spesso valore informativo letterale. Ingestione e query seguono quindi lo stesso
schema operativo: in scrittura vengono generati embedding densi e sparsi dei
documenti; in lettura la query viene trasformata nelle due rappresentazioni
corrispondenti e inviata a
...
servizi nel file \texttt{dev\_containers/docker-compose.yml}. Il retrieval
semantico viene così introdotto senza modificare il modello dati principale né
...
\begin{table}[ht]
  ...
  \begin{tabular}{L{4.1cm}L{3.6cm}L{5.1cm}}
    ...
    \breakcode{FASTEMBED_SPARSE_MODEL} & \breakcode{Qdrant/bm25} &
