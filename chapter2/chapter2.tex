...
\todo{occhio che l'intestazione di queste pagine sovrappone le scritte, vedi se ci puoi fare qualcosa}
...
\begin{itemize}
  ...
  \item \textbf{Ciclo temporale (Epoch)}: le Gilde si ricreano ogni quattro
    mesi; ogni ciclo costituisce un'``epocà' distinta, con nuovi temi e nuovi
...
tecnologie e termini ricorrenti nelle
...
nelle due rappresentazioni corrispondenti e inviata al retriever ibrido
Qdrant~\cite{haystackQdrantHybrid2026}. Il valore
...
Nel contesto i3Guild, molte query contengono parole ricorrenti come ``gildà',
``fondarè' o ``consigli''. Questi termini descrivono l'interazione con il
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{5.5cm}p{2.5cm}p{4cm}}
    ...
    ``intelligenza artificialè' & \texttt{intelligenza artificiale} &
      (nessun rewriting) \\
...
\todo{ma qualche screenshot del sistema, per far vedere che esiste quello che hai fatto? giusto un paio magari, per lo scenario}
...
\begin{table}[ht]
  \centering
  \caption{Configurazione esterna della componente RAG.}
  \label{tab:env_vars}
  \small
  \begin{tabular}{L{4.3cm}L{8.3cm}}
    \toprule
    \textbf{Gruppo di variabili} & \textbf{Ruolo} \\
    \midrule
    Connessione Qdrant & Endpoint del database vettoriale e nome della
      collection principale. \\
    Modello LLM & Endpoint Ollama e modello di chat configurabile, con
      \texttt{gemma3:4b} come scelta adottata nel prototipo. \\
    Embedding & Modelli FastEmbed denso e sparso, dimensione del vettore denso
      e directory di cache locale. \\
    Retrieval & Parametri di soglia, numero massimo di documenti, cache
      in-process e opzioni di query rewriting. \\
    Integrazione applicativa & Endpoint interno del servizio RAG usato da
      Next.js e timeout delle chiamate di generazione. \\
    \bottomrule
  \end{tabular}
\end{table}
