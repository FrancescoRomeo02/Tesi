...
\begin{itemize}
  ...
  \item \textbf{Limitazioni hardware}: l'ambiente Docker dispone di 8~GB di
    RAM complessivi, condivisi tra database, servizi applicativi, pipeline di
    embedding quando attiva e modello di generazione del testo.
    Questo vincolo è determinante per la selezione del modello LLM (Sezione~\ref{subsec:llm_choice}).
  \item \textbf{No API esterne per LLM ed embedding}: generazione ed embedding
    devono restare locali; Ollama gestisce il modello generativo, mentre
    FastEmbed/Haystack gestisce i modelli di embedding senza dipendere da API
    di provider esterni.
\end{itemize}
...
(\texttt{QdrantDocumentStore}), FastEmbed per embedding densi e sparsi e
Ollama per la generazione, il minore overhead di orchestrazione e la
serializzabilità delle pipeline rendono Haystack la scelta più adeguata per un
...
\begin{table}[ht]
  ...
  \small
  \begin{tabular}{p{3cm}p{4cm}p{2.5cm}p{4cm}}
    \toprule
    \textbf{Ruolo} & \textbf{Componente Haystack} & \textbf{Default} &
      \textbf{Funzione} \\
    \midrule
    Embedding denso & \texttt{FastembedDocumentEmbedder} /
      \texttt{FastembedTextEmbedder} & \texttt{BAAI/bge-base-en-v1.5} &
      Similarità semantica tra query e documenti. \\
    Embedding sparso & \texttt{FastembedSparseDocumentEmbedder} /
      \texttt{FastembedSparseTextEmbedder} & \texttt{prithivida/Splade\_PP\_en\_v1} &
      Matching lessicale pesato, utile per termini specifici. \\
    Store ibrido & \texttt{QdrantDocumentStore} con
      \texttt{use\_sparse\_embeddings=true} & \texttt{i3guild\_knowledge} &
      Persistenza con vettori densi e rappresentazioni sparse. \\
    \bottomrule
  \end{tabular}
\end{table}

\noindent La scelta aggiornata risponde a tre esigenze. La prima è eliminare la
dipendenza da Ollama nella fase batch di indicizzazione: Ollama resta dedicato
alla generazione LLM, mentre FastEmbed produce gli embedding in un job separato.
La seconda è rendere il retrieval più robusto rispetto alle sole similarità
dense. La terza è avvicinare ingestione e query allo stesso schema operativo:
in scrittura vengono generati embedding densi e sparsi dei documenti; in lettura
la query viene trasformata nelle due rappresentazioni corrispondenti e inviata a
...
\noindent \textbf{Scelta motivata: \texttt{gemma3:1b}.} Il modello adottato nel
prototipo è \texttt{gemma3:1b}. La scelta deriva da prove preliminari interne
su modelli disponibili nel registry Ollama, valutati rispetto a tre criteri:
latenza percepita durante l'interazione, consumo di memoria compatibile con gli
altri container e qualità minima delle risposte in presenza di contesto RAG. Le
motivazioni principali sono:

\begin{itemize}
  \item \textbf{RAM compatibile con i vincoli}: il modello può essere caricato
    senza saturare le risorse condivise con PostgreSQL, Qdrant, Next.js e il
    servizio RAG.
  \item \textbf{Latenza accettabile}: i tempi di risposta sono compatibili con
    un uso interattivo del chatbot, tenendo conto del cold start del modello.
  \item \textbf{Qualità sufficiente per il prototipo}: le risposte risultano
    adeguate quando il modello riceve contesto recuperato da Qdrant.
  \item \textbf{Integrazione semplice}: il modello è disponibile direttamente in
    Ollama e non richiede conversioni o runtime aggiuntivi.
\end{itemize}
...
\begin{description}
  ...
  \item[Orchestrazione RAG (Haystack 2.x)] Haystack è usato sia nel job di
    indicizzazione sia nel servizio di query. La pipeline batch collega
    \texttt{FastembedDocumentEmbedder}, \texttt{FastembedSparseDocumentEmbedder}
    e \texttt{DocumentWriter}; la pipeline di query collega
    \texttt{FastembedTextEmbedder}, \texttt{FastembedSparseTextEmbedder} e
    \texttt{QdrantHybridRetriever}.
...
\begin{table}[ht]
  ...
  \begin{tabular}{lll}
    ...
    \midrule
    \texttt{embedding\_dim} & 768 & Dimensione della componente densa FastEmbed \\
    \texttt{vectors.distance} & \texttt{Cosine} & Standard per similarità
      semantica testuale \\
    \texttt{use\_sparse\_embeddings} & \texttt{true} & Abilita la componente
      sparsa per retrieval ibrido \\
    \texttt{recreate\_index} & \texttt{true} (job batch) & Ricrea collection
...
\begin{enumerate}
  ...
  \item \textbf{Transform}: normalizzazione HTML tramite \texttt{BeautifulSoup},
    ...
    overlap quando il testo supera \texttt{INGEST\_MAX\_DOCUMENT\_CHARS}.
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{2.5cm}p{2.5cm}p{1.5cm}p{5cm}}
    ...
    \texttt{i3guild\_embedding\_pipeline} & Build locale & -- & Job batch
      PostgreSQL $\rightarrow$ FastEmbed/Haystack $\rightarrow$ Qdrant \\
    \texttt{postgres} & già presente & già presente & Sorgente autoritativa
...
\begin{table}[ht]
  ...
  \begin{tabular}{l l p{5cm}}
    ...
    \texttt{OLLAMA\_LLM\_MODEL} / \texttt{OLLAMA\_CHAT\_MODEL} &
      \texttt{gemma3:1b} / configurabile & Modello LLM usato dal servizio o
      inoltrato dal frontend \\
    \texttt{EMBEDDING\_DIMENSIONS} & \texttt{768} & Dimensione vettore denso \\
    \texttt{FASTEMBED\_DENSE\_MODEL} & \texttt{BAAI/bge-base-en-v1.5} &
      Modello FastEmbed denso \\
    \texttt{FASTEMBED\_SPARSE\_MODEL} & \texttt{prithivida/Splade\_PP\_en\_v1} &
      Modello FastEmbed sparso \\
    \texttt{RAG\_CACHE\_TTL\_SECONDS} & \texttt{60} & TTL cache retrieval
      in-process \\
...
\texttt{src/lib/services/rag.service.ts}. Il client espone funzioni per
