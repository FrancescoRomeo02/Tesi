...
Le \textbf{gilde} (\emph{gilde}) sono una delle strategie principali con cui
Intré rende operativa la propria idea di apprendimento organizzativo e continuo. Sono cicli
...
proposta raccoglie un numero minimo di partecipanti, indcativamente almeno 4; il gruppo lavora in
autogestione e dedica al tema scelto una quota fissa del proprio tempo
lavorativo. Il riusltato raggiunto dalla gilda sarà poi presentato al Camp successivo. Il modello combina quindi
...
e~\ref{tab:field_stats}. Questi valori non hanno solo funzione descrittiva ma sono necessari 
per determinare la granularità dell'indicizzazione, la strategia di chunking e la
scelta del modello di embedding più adeguato.
...
Questa distinzione separa in modo netto contenuto da metadato. I campi testuali della Gilda
alimentano l'embedding e la ricerca semantica; epoch, partecipanti, modalità di
partecipazione e identificativi restano nel payload strutturato e servono per
filtrare o contestualizzare la risposta ottenuta. La separazione evita due problemi:
...
numeriche interrogabili in Qdrant. Nel progetto questa funzione è separata
...
pipeline genera, come anticipato, sia embedding densi sia embedding sparsi, così il retrieval può
...
varianti quantizzate a 4 e 8 bit. Nel prototipo realizzato la scelta del modello
di chat è stata condizionata da un vincolo operativo preciso: l'intero stack
Docker dispone di risorse limitate (12GB di RAM) e deve eseguire, nello stesso ambiente,
PostgreSQL, Qdrant, il servizio Next.js, la pipeline RAG, Ollama e tutti gli altri microservizi sviluppati nel tempo. 
Per questo la selezione non mira a identificare il modello migliore in senso assoluto, ma
...
PostgreSQL, Qdrant, Next.js e FastAPI, ma offre una prestazione qualitativa maggiore
...
sistema sviluppato non adotta un valore
\texttt{top\_k} fisso, una soglia rigida rischierebbe di includere
...
strategia utilizzata è il \textbf{full re-index periodico}: il job di
...
Il capitolo illustra come il caso d'uso i3Guild è stato tradotto in una proposta architetturale
coerente con i vincoli imposti. Si è notato come il corpus delle gilde è abbastanza contenuto
...
Le decisioni principali restano guidate dall'esigenza di mantenere i
