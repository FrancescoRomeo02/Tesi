...
Le scelte tecnologiche descritte portano a un'architettura modulare e containerizzata. 
Il sistema separa le principali fasi operative: ingestione e normalizzazione dei dati, 
generazione degli embedding, persistenza vettoriale, retrieval e inferenza tramite LLM. 
Questa separazione rende i componenti più facili da monitorare e sostituire, mantenendo 
l'integrazione coerente con lo stack i3Guild. La Figura~\ref{fig:architecture} mostra lo 
schema generale dell'architettura. 
...
lessicale. Se la stringa estratta è non vuota e diversa dalla query originale
normalizzata, il retrieval usa tale stringa come query di ricerca; altrimenti
mantiene la query originale. La Tabella~\ref{tab:query_rewriting} riporta alcuni
esempi coerenti con questo comportamento.
...
La configurazione esterna del servizio è riassunta nella
Tabella~\ref{tab:env_vars}; i gruppi indicati separano connessione, modello,
embedding, retrieval e integrazione applicativa.
