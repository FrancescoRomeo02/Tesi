...
PostgreSQL~\cite{postgresql2024}. Il secondo è
\texttt{i3guild-embedding-pipeline}, responsabile dell'estrazione dei dati,
della normalizzazione testuale, della generazione di embedding densi e sparsi
con FastEmbed/Haystack e dell'upsert su Qdrant. Il terzo è
\texttt{i3guild-rag}, un microservizio Python che espone le API di retrieval e
chat. Questa separazione riduce l'accoppiamento tra applicazione e componente
RAG: i3Guild rimane la sorgente autoritativa dei dati, Qdrant conserva la
knowledge base ibrida e Ollama viene usato solo per la generazione
LLM~\cite{qdrant2023,ollama2023}.
...
La configurazione avviene tramite variabili d'ambiente. Le più rilevanti
definiscono connessione a PostgreSQL, endpoint e collection Qdrant, modelli
FastEmbed e dimensione del vettore denso. Il modulo legge anche parametri
operativi per ricreazione dell'indice, dimensione dei batch, soglia di chunking
e overlap. La pipeline non richiede più variabili Ollama per l'indicizzazione.
...
funzione di costruzione compone un testo con sezioni marcate per sommario,
obiettivi, rischi, risultati raggiunti e partecipanti. I campi HTML vengono
normalizzati con BeautifulSoup~\cite{beautifulsoup2024}: il codice rimuove i
tag, conserva il testo e compatta gli spazi. Per il retrieval serve un testo
pulito e stabile, non una resa editoriale. Uno schema semplificato della
costruzione del documento è riportato
...
La fase finale costruisce una pipeline Haystack con un embedder denso, un
embedder sparso e un writer. Il primo componente aggiunge ai documenti la
rappresentazione densa; il secondo aggiunge la rappresentazione sparsa; il writer
persiste entrambi i segnali nel document store Qdrant. La configurazione
essenziale della pipeline è riportata in Appendice~\ref{app:B},
...
\texttt{i3guild-rag}. Espone endpoint per retrieval, chat sincrona e chat in
streaming. La configurazione è centralizzata nel modulo delle impostazioni e usa
\texttt{pydantic-settings}; i valori di default
...
endpoint principali. Il sistema
...
La risposta della route è NDJSON streaming: invia eventi di stato periodici
mentre il modello lavora e conclude con un evento \texttt{draft} oppure con un
evento \texttt{error}. Questa scelta evita che un gateway con timeout aggressivo
interrompa operazioni LLM lente prima che il frontend riceva un esito
interpretabile.
...
La pipeline di embedding prepara l'indice ibrido a partire da PostgreSQL; il
microservizio RAG espone retrieval e generazione; Qdrant gestisce vettori densi,
rappresentazioni sparse e metadati; Ollama mantiene l'inferenza generativa in
locale; Next.js integra chat, generazione bozza e fallback applicativi. Le parti
più specifiche del lavoro sono la normalizzazione del corpus i3Guild, il
retrieval ibrido con query rewriting, cache e deduplicazione, e la separazione
tra chat RAG e generazione strutturata senza retrieval. Il workflow agentico è
stato trattato come una linea sperimentale e non come soluzione finale, perché a
parità di hardware introduceva maggiore complessità e maggiore costo
computazionale. Queste scelte rendono il prototipo coerente con i vincoli di
