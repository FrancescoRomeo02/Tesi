...
RAG espone invece le API Python per retrieval e chat. Questa separazione riduce
...
stabilire quando applicare il chunking con overlap. La generazione degli embedding 
avviene attraverso i modelli mostrati nella Tabella~\ref{tab:embedding_models}
così da non dipendere da Ollama.
...
tramite variabile d'ambiente. Questa scelta riduce il picco di memoria dello
sparse embedder e mantiene il comportamento batch del container: il job parte,
scrive su Qdrant e termina. La normalizzazione dei campi avviene prima della
costruzione dei documenti, rendendo esplicito quali testi entrano nella
knowledge base.
...
\texttt{max\_documents=10} e \texttt{gap\_threshold=0.08}. La risposta non
contiene soltanto i documenti selezionati, ma anche informazioni diagnostiche:
numero di candidati iniziali, soglia usata, motivo del taglio, eventuale query
riscritta e filtri estratti automaticamente. Lo schema della richiesta è
riportato in Appendice~\ref{app:B}, Listato~\ref{lst:retrieve_request}.
...
Questa scelta evita di fissare a priori un unico valore di \texttt{top\_k}. Per
...
con un evento \texttt{error}. Questa scelta evita che un gateway con timeout
aggressivo interrompa operazioni LLM lente prima che il frontend riceva un esito
interpretabile.
...
candidati, documenti restituiti e log di timeout verso Ollama. Questi campi non
sono solo utili per il debug. Permettono di ricostruire il comportamento del
sistema durante la valutazione applicativa: quale query è stata effettivamente
cercata, quanti documenti sono stati recuperati, perché alcuni risultati sono
stati scartati e se il collo di bottiglia si trova nel retrieval o nella
generazione.
