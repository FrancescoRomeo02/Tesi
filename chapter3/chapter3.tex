...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[
    ...
    \node[box, right=of pipeline] (qdrant) {Qdrant\\indice ibrido};
    \node[box, below=of qdrant] (rag) {FastAPI RAG\\retrieval e chat};
    \node[box, left=of rag] (ollama) {Ollama\\generazione LLM};
...
\texttt{FASTEMBED\_DENSE\_MODEL}, \texttt{FASTEMBED\_SPARSE\_MODEL} ed
\texttt{EMBEDDING\_DIMENSIONS}. Il modulo legge anche parametri operativi come
\texttt{RECREATE\_INDEX}, \texttt{INGEST\_PIPELINE\_BATCH\_SIZE},
\texttt{INGEST\_MAX\_DOCUMENT\_CHARS} e
\texttt{INGEST\_CHUNK\_OVERLAP\_CHARS}. La pipeline non richiede più
\texttt{OLLAMA\_BASE\_URL} o \texttt{OLLAMA\_EMBEDDING\_MODEL} per
l'indicizzazione.
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{4.1cm}p{4.3cm}p{5cm}}
    ...
    \texttt{FASTEMBED\_DENSE\_MODEL}
      & FastEmbed
      & Modello usato per gli embedding densi. \\
    \texttt{FASTEMBED\_SPARSE\_MODEL}
      & FastEmbed
      & Modello usato per gli embedding sparsi. \\
    \texttt{FASTEMBED\_CACHE\_DIR}
      & FastEmbed
    ...
    \texttt{RECREATE\_INDEX}
      ...
      & Ricrea la collection; nella versione aggiornata il default è
        \texttt{true}. \\
    \texttt{INGEST\_PIPELINE\_BATCH\_SIZE}
      & Operatività
...
\texttt{haystack.Document}. La funzione \texttt{create\_documents\_from\_row()}
...
La fase finale costruisce una pipeline Haystack composta da
\texttt{FastembedDocumentEmbedder}, \texttt{FastembedSparseDocumentEmbedder} e
\texttt{DocumentWriter}. Il primo componente aggiunge ai documenti la
rappresentazione densa; il secondo aggiunge la rappresentazione sparsa; il writer
persiste entrambi i segnali nel \texttt{QdrantDocumentStore}. La configurazione
essenziale della pipeline è riportata in Appendice~\ref{app:B},
Listato~\ref{lst:embedding_pipeline_impl}.

La pipeline viene eseguita in batch applicativi, configurati tramite
\texttt{INGEST\_PIPELINE\_BATCH\_SIZE}. Questa scelta riduce il picco di memoria
dello sparse embedder e mantiene il comportamento batch del container: il job
...
streaming. La configurazione è centralizzata in
...
densi di dimensione 768 con similarità coseno e abilita
\texttt{use\_sparse\_embeddings}. La query utente viene elaborata da due
