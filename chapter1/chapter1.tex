...
Il capitolo non ha l'obiettivo di esaurire lo stato dell'arte sull'inferenza
locale o sulla quantizzazione. Questi temi diventano oggetto specifico dei
Capitoli~\ref{chap:ottimizzazione} e~\ref{chap:evaluation}. Qui vengono
presentati solo nella misura necessaria a comprendere il prototipo realizzato in
azienda e le sue scelte progettuali.
...
\item \textbf{Generazione degli embedding.} Ogni chunk viene trasformato in due
rappresentazioni complementari. La prima è un vettore denso prodotto da un
modello di embedding $f\colon \mathcal{T} \rightarrow \mathbb{R}^d$, in modo
che documenti semanticamente simili producano vettori vicini secondo la
similarità coseno:
...
La seconda è una rappresentazione sparsa, nella quale il testo è descritto da
pesi associati a termini o feature lessicali. Questa componente preserva segnali
che l'embedding denso può attenuare, ad esempio nomi propri, tecnologie,
sigle o termini specifici del dominio.

\item \textbf{Indicizzazione nel database vettoriale.} Le rappresentazioni
dense e sparse vengono persistite in \textbf{Qdrant}~\cite{qdrant2023}, insieme
ai metadati strutturati necessari al filtraggio. Per la componente densa Qdrant
usa l'algoritmo HNSW (\textit{Hierarchical Navigable Small World})
~\cite{malkov2018efficient} per la ricerca approssimata
(\textit{Approximate Nearest Neighbor}, ANN); la componente sparsa abilita il
matching lessicale pesato. La scelta di Qdrant rispetto ad alternative quali
pgvector, Chroma e Milvus è motivata nel Capitolo~\ref{chap:progettazione}.
\end{itemize}
...
\begin{itemize}
\item \textbf{Embedding della query.} La query dell'utente viene trasformata
nelle stesse due rappresentazioni usate in indicizzazione: un embedding denso
per la similarità semantica e una rappresentazione sparsa per il matching
lessicale.

\item \textbf{Retrieval.} Si esegue una ricerca ibrida in Qdrant combinando il
punteggio denso e quello sparso. La ricerca può essere limitata tramite
...
Il presente capitolo ha fornito il quadro tecnico necessario a comprendere il
progetto aziendale descritto nei capitoli successivi. Le architetture
Transformer costituiscono il substrato comune a generazione ed embedding; i loro
limiti in contesti a conoscenza chiusa, cioè conoscenza statica, allucinazioni e
assenza di accesso nativo ai dati interni, motivano l'adozione del paradigma
RAG. La scelta di operare con modelli e infrastrutture on-premise, come Ollama,
Qdrant e Haystack, risponde ai vincoli di riservatezza e manutenibilità del
caso i3Guild. Il capitolo successivo traduce questi fondamenti in requisiti,
scelte tecnologiche e architettura del sistema.
